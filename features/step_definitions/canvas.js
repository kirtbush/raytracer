...
(function (canvas_1) {
    ...
    var Canvas = /** @class */ (function () {
        ...
        Canvas.prototype.write_pixel = function (x, y, c) {
    ...
    function scale(value, scale) {
        var newVal = Math.round(value * scale);
        if (newVal < 0)
            newVal = 0;
    ...
    function canvas_to_ppm(canvas1) {
        var ppmStr = create_ppm_header(canvas1.width, canvas1.height, DEFAULT_COLOR_SCALE);
        var idx = 0, idy = 0;
        var dataLines = [];
        var nextLine = "\n";
        var s = { dl: dataLines, nl: nextLine };
        for (idy = 0; idy < canvas1.height; idy++) {
            for (idx = 0; idx < canvas1.width; idx++) {
                var r = canvas1.pixels[idx][idy].red;
                s = add_color_to_str(nextLine, dataLines, r);
                nextLine = s.nl;
                dataLines = s.dl;
                var g = canvas1.pixels[idx][idy].green;
                s = add_color_to_str(nextLine, dataLines, g);
                nextLine = s.nl;
                dataLines = s.dl;
                var b = canvas1.pixels[idx][idy].blue;
